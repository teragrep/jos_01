...
public class ProcessStat implements Status {
    ...
    public ProcessStat(ArrayList<String> rows) {
        ...
        for (String row : rows) {
            String[] fields = row.split(" (?![^()]*\\))"); // Regex matching spacebars except for within parentheses in case "comm" field contains spacebars
            for (int i = 0; i < fields.length; i++) {
                statistics.put(ProcessStatFields.values()[i].name(), fields[i]);
            }
        }
